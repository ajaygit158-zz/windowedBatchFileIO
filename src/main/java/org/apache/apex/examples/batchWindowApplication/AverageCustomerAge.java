...
public class AverageCustomerAge implements Accumulation<Customer, MutablePair<Double, Long>, Double>
{
  @Override
  ...
  @Override
  public MutablePair<Double, Long> accumulate(MutablePair<Double, Long> accu, Customer input)
  {
    accu.setLeft(accu.getLeft() * ((double)accu.getRight() / (accu.getRight() + 1))
        + input.getAge() * 1.0 / (accu.getRight() + 1));
